...
function appendRow() {
    ...
    if (currentRowsInTbody < 10) {
        ...
        let columnsToCreate;
        if (currentRowsInTbody === 0) columnsToCreate = 2; // Створюємо з мінімальною к-тю стовпчиків
        else columnsToCreate = tbody.rows[0].cells.length;
    ...
    if (colCount > 2) {
        for (const row of tbody.rows){
            if (row.cells.length > 0) {
                if (row.lastElementChild && row.lastElementChild.tagName === "TD") row.lastElementChild.remove();
            }
        }
        updateButtonStates();
    }
